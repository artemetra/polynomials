...
class Polynomial:
    ...
    def _term_mul(self, coeff, power) -> Polynomial:
        ...
        terms = [
            (coeff*c, p+power) for c, p in self.coeff_and_power()
        ]
    ...
    def __mul__(self, other) -> Polynomial:
        other = Polynomial._number_to_poly(other)
        h = [
            self._term_mul(c, p) for c, p in other.coeff_and_power()
        ]
        s = Polynomial([0])
        for p in h:
            s += p
    ...
    def evaluate(self, x):
        return sum(
            coeff*(x**power) for coeff, power in self.coeff_and_power()
        )
